export default function InputGroups({ itemGroups, itemPrefix, config, setConfig }: { itemGroups: { title: string, description: string, items: { key: string, name: string, placeholder?: string }[] }[], itemPrefix?: string, config: any, setConfig: React.Dispatch<React.SetStateAction<any>> }) {
                            ...
                            <div className="col-span-full">
                                ...
                                <div className="mt-2 px-4">
                                    <div className="dark:ring-zinc-700 flex rounded-md shadow-sm ring-1 ring-inset ring-zinc-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-full">
                                        {itemPrefix && <span className="flex select-none items-center pl-3 text-zinc-500 sm:text-sm">{itemPrefix}</span>}
                                        <input
                                            type="text"
                                            name={item.key}
                                            id={item.key}
                                            autoComplete={item.key}
                                            onChange={(e) => setConfig({ ...config, [item.key]: mergePrefix(itemPrefix, e.target.value) })}
                                            value={purgePrefix(itemPrefix, config[item.key])}
